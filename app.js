...
loadPosts.use(function(req, res, next){
    fs.readFile('public/texts/slogan.txt', function (err, slogandata){
	if (err){console.log("didn't read slogandata");}
	else{
    ...
    fs.readFile('public/texts/stories.json', function(err, data){
	if (err){console.log("didn't read stories");}
	else{
	    var posts = data;
	    ...
	    res.render('jumbotron.ejs', {headline: "Composite Drilling", postlength: title.length, posttitle: title, postslug: slug, postcontent: content, slogan: res.slogan});
	}
	next();
    });
...
app.get('/', function(req, res){
 res.render('cover.ejs', {heading: "Welcome"});

//    res.render('cover.ejs', {title:'Smarter drilling', heading: 'Say welcome to faster and safer operations'});
